    ...
    stages {
        ...
        stage('Massive Log Collection') {
            steps {
                ...
                echo ===== Network Info =====
                docker network ls > logs/network_list.txt
                docker inspect $(docker ps -q) > logs/network_inspect.txt
                docker exec %APP_CONTAINER% ping -n 3 %DB_CONTAINER% > logs/network_ping.txt 2>&1
